...
const CODE_MESSAGES: Record<string, string> = {
  ...
  QUOTA_EXCEEDED: "本月 TTS Token 配额已用完——请下月再试或联系运营",
  UNAUTHORIZED: "登录已过期——请重新登录",
  FORBIDDEN: "无权执行此操作",
  VALIDATION_ERROR: "请求参数有误——请检查音色 ID 与台本格式",
  HTTP_ERROR: "服务暂时不可用——请确认平台 API 是否在运行",
  INTERNAL_ERROR: "服务器内部错误——请稍后重试；若刚完成快速克隆，请刷新页面后再点「开始合成」",
  ...
  ROLE_NOT_FOUND: "角色绑定不存在——可能已被解绑，请刷新页面",
};
...
export function formatApiError(e: unknown): string {
  if (e instanceof ApiError) {
    ...
    if (e.status === 405) return "接口方法不可用——请重启平台 API（.\\scripts\\platform_start.ps1）后再试";
    if (e.status === 422) return `${CODE_MESSAGES.VALIDATION_ERROR}（${e.message}）`;
    if (e.status === 404) return "接口不存在——请重启 API 并确认已执行最新迁移";
    if (e.status >= 500 || e.code === "INTERNAL_ERROR") {
      return CODE_MESSAGES.INTERNAL_ERROR ?? e.message;
    }
