...
| A-FR-07 | 合成成功按文本长度（含标点）扣减字符配额；训练成功或**导入外部权重成功**各扣减 1 次训练配额 | P0 | TC-S03 |
...
| 属性 | 说明 |
|------|------|
| **输入来源** | 用户触发 `POST /api/v1/voices/{id}/train` 或 `POST /api/v1/voices/import-weights`（含 upload）；系统读取当月已用次数 |
| **数量** | 每次训练请求计 1 次 |
| **度量单位** | 次/自然月 |
...
| 配额查询 | GET | `/api/v1/usage/quota` | ✅ W1 已实现 |
...
| TBD-A05 | ~~独立 `GET /api/v1/usage/quota` 是否 W1 交付~~ | ✅ W1 已交付 |
